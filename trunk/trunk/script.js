...
function Params(A) {
	...
	for(var i = 0; i < A.length; ++i) {
		...
		if(isProp) {
			...
			if(cur=="&") {
				obj[curProp] = decodeURIComponent(curValue);
				curValue = "";
				curProp = "";
				isProp = true;
				continue;
...
var opts  = {
	vq : new Array("Max Quality", ["360p", "480p", "720p", "1080p"], "Please choose the maximum video quality your computer can handle"),
...
for(var arg in swfArgs) if(arg.indexOf("ad_")==0) delete swfArgs[arg];
/*
	swfArgs.cc_load_policy = "1";
	swfArgs.cc_font = "Arial Unicode MS, arial, verdana, _sans";
*/
swfArgs.vq=["hd1080", "hd720", "large", "medium"][opts.vq];
